...
def train(train_data_loader, eval_data_loader, model, optimizer, num_epoch, log_step_interval, save_step_interval, eval_step_interval, save_path, resume=""):
    ...
    if resume != "":
        ...
        checkpoint = torch.load(resume, map_location=torch.device("cpu")) #可以是cpu,cuda,cuda:index 需要加入这个函数map_location用来加载
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        start_epoch = checkpoint['epoch']
        start_step = checkpoint['step']

    model.cuda() # 模型拷贝 原地操作模型的参数拷贝在gpu上 不需要model = model.cdua
    ...
    for epoch_index in range(start_epoch, num_epoch):
        ...
        for batch_index, (target, token_index) in enumerate(train_data_loader):
            ...
            token_index = token_index.cuda() # 数据拷贝 tensor.cuda 函数
...
if __name__ == "__main__":
    if torch.cuda.is_available(): # 判断cuda是否可用
        ...
        os.environ["CUDA_VISIBLE_DEVICES"] = "0" # 限制GPU的卡的使用 仅对gpu0 可见 也可以在命令行 在python命令之前
    else:
